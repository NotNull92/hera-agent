...
// `doctor --agent-rules` can extract the TL;DR + Pitfalls subset without
// requiring repo access. The canonical file lives at the repo root; a
// copy at cmd/AGENT.md exists because //go:embed cannot escape the
// package directory. A regression test asserts the two stay in sync.
//
//go:embed AGENT.md
...
// `hera-agent doctor --agent-rules >> <your-rules-file>`.
func extractAgentRules() string {
	var out strings.Builder
	out.WriteString("# hera-agent — Quick Rules + Pitfalls\n\n")
	out.WriteString("> Emitted by `hera-agent doctor --agent-rules`. ")
	out.WriteString("Works with any AI coding agent (Claude Code, Codex, Cursor, Copilot, ...). ")
	out.WriteString("Full guide: https://github.com/NotNull92/hera-agent/blob/main/AGENT.md\n\n")
