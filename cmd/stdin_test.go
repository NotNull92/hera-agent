...
func TestReadStdinIfPiped_RealPipeReadsCode(t *testing.T) {
	// Real piped input ("echo 'code' | hera-agent exec") must still be consumed
	// and prepended as the positional arg.
	r, w, err := os.Pipe()
	if err != nil {
		t.Fatalf("pipe: %v", err)
	}
...
func TestReadStdinIfPiped_FileRedirectReadsCode(t *testing.T) {
	// `hera-agent exec < code.cs` — stdin is a regular file. Must read it.
