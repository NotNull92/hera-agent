# hera-agent — Guide for AI Agents

> You are an AI coding agent operating in a Unity project that has `hera-agent` available as a CLI. This document tells you how to use it efficiently. It is meant to be loaded into your project's rules file so every session has it without spending tokens to discover it.
>
...
>
> Or run `hera-agent doctor --agent-rules >> <your rules file>` to append the lean subset.

`hera-agent` is a CLI that drives a running Unity Editor over HTTP. Common uses: execute C# inside the Editor, read console logs, query the active scene, run tests, capture screenshots. Each call is a tool round-trip; response bytes become your input tokens, so reads cost as much as your own writes.

Quick links:
- Full command catalog → [`docs/COMMANDS.md`](docs/COMMANDS.md)
- Architecture → [`docs/ARCHITECTURE.md`](docs/ARCHITECTURE.md)
- Custom tool authoring → `hera-agent help custom-tools`
...
// Error envelope shape
{
  ...
  "message": "Compile error: L15 CS1525: ... (+2 more)",
  "data": { "compile_errors": [ ... ] }
}
```

**[Rule 4]** Batch related operations into a single `exec` call. Each call has fixed envelope + HTTP overhead; one `exec` that creates Canvas + 3 buttons is cheaper than three separate calls. For coarser composition use the `batch` command.
...
| List all tools | `list` | `--names` for token-efficient name list, `--tool <name>` for full schema. |
...
```bash
hera-agent exec "var x = SomeType.SomeMethod();" --check
```
...
```bash
echo 'return Application.dataPath;' | hera-agent exec
```
...
```bash
hera-agent console --type error --lines 5
```
...
```bash
hera-agent exec --check "var x = MyType.MaybeRenamed();"
```
...
### 3.7 One-shot validate + run via batch

```bash
echo '{"commands":[
  {"command":"exec", "params":{"code":"return Application.unityVersion;"}},
  {"command":"editor", "params":{"action":"refresh","compile":true}}
]}' | hera-agent batch
```

`fail_fast: true` (default) stops on first failure. Use `fail_fast: false` when you want every command attempted regardless. Note: batch is sequential — no branching, no data passing between steps. For complex flows, use multiple `exec` calls.
...
Fixed in **v0.0.25+**. If you're on v0.0.24 or earlier and `exec` hangs in Cursor's shell / bash `$()` / CI, either upgrade (`hera-agent update`) or prepend `$null |` (PowerShell) / `</dev/null` (bash) to every `exec` call.
...
| `list` | List registered tools | `--names`, `--tool <name>` |
| `batch` | Run multiple commands | `--file path.json`, or pipe JSON |
...
| Variable | Effect |
|---|---|
| `HERA_AGENT_QUIET=1` | Treat output as agent-target even in a real TTY (compact JSON, no decoration). |
| `HERA_AGENT_QUIET=0` | Force human mode even when stdout is piped. |
| `HERA_AGENT_NO_PATH_CHECK=1` | Silence the implicit per-command warning about PATH mismatch (useful when calling from a wrapper binary). |
| `--port N` / `--project <path>` | Select among multiple open Unity instances. |
| `--timeout <ms>` | Per-call HTTP timeout (default 60000). |

### 5.4 What `--depth` actually does
...
If something here contradicts what `hera-agent <cmd> --help` says, trust `--help`. This guide is a curated subset, not the authoritative reference. The catalog at `docs/COMMANDS.md` is also authoritative for flag tables.

If you find a real bug or want to suggest a pattern, file an issue at `https://github.com/NotNull92/hera-agent/issues`.
