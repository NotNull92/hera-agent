# Issue: PowerShell에서 `unity-agent-cli-pro exec` 인자 파싱 실패

- **상태**: Open
- **심각도**: Medium (워크어라운드 존재하나, DX 저해)
- **발견일**: 2026-05-11
- **환경**: Windows 11 Pro, PowerShell 7+, unity-agent-cli-pro (Go 바이너리)

---

## 증상

PowerShell에서 `unity-agent-cli-pro exec "C# code"` 형태로 실행하면 아래 에러 발생:

```
unity-agent-cli-pro.exe: The command parameter was already specified.
```
...
unity-agent-cli-pro exec "var x = 1; return x.ToString();"

# 케이스 2: $를 PowerShell 변수 참조로 해석함
unity-agent-cli-pro exec "return $\"Hello {name}\";"

# 케이스 3: 중괄호가 PowerShell ScriptBlock으로 해석됨
unity-agent-cli-pro exec "var dict = new Dictionary<string, int>(){};  return dict.Count;"
```

### 성공하는 명령어 (Bash stdin pipe)

```bash
echo 'var x = 1; return x.ToString();' | unity-agent-cli-pro exec
```
...
PowerShell은 더블 쿼트(`"`) 내부에서도 다음 문자들을 **자체 문법으로 해석**한다:
...
`unity-agent-cli-pro exec`는 `os.Args` 또는 cobra/flag 라이브러리로 인자를 받는다. PowerShell이 문자열을 분리하여 넘기면 CLI는 이미 분리된 복수 인자를 받게 되어 `"The command parameter was already specified"` 에러를 출력한다.
...
```bash
echo 'var enemy = UnityEngine.Object.FindFirstObjectByType<Enemy>(); if (enemy == null) return "not found"; return enemy.name;' | unity-agent-cli-pro exec
```
...
```powershell
# PowerShell에서 파이프
'var x = 1; return x;' | unity-agent-cli-pro exec --stdin

# 또는 파일에서 읽기
Get-Content script.csx | unity-agent-cli-pro exec --stdin
```
...
```powershell
unity-agent-cli-pro exec --file check_enemy.csx
```
...
Try one of:
  1. Use single quotes:  unity-agent-cli-pro exec 'your code here;'
  2. Use stdin pipe:     echo 'your code' | unity-agent-cli-pro exec
  3. Use here-string:    unity-agent-cli-pro exec @'
                         your code here;
                         '@
```
...
- `unity-agent-cli-pro exec` 명령어
- PowerShell 7+ 환경 (Windows 기본 셸)
- Claude Code CLI가 PowerShell을 통해 unity-agent-cli-pro를 호출하는 모든 시나리오
